...
def main():
    ...
    with open(TXT_OUT, 'w', newline='') as f:
        csv_wr = csv.writer(f)
        csv_wr.writerow(['frame', 'visible', 'x', 'y'])   # 1. 換 header
        ...
        while True:
            ...
            if not initiated:
                if has_obs:
                    ...
                    # ❗ 這裡已經沒觀測，但要先寫 0 再 continue
                    csv_wr.writerow([frame_id, 0, -1, -1])   # 2. 先寫 visible=0
                    writer.write(img)
                    continue
            else:
                kf.predict()
                ...
                if miss_count > MAX_MISS:
                    initiated = False
                    csv_wr.writerow([frame_id, 0, -1, -1])   # 3. 先寫 visible=0
                    traj.clear()
                    writer.write(img)
                    continue
            ...
            # 4. 保證不論哪條路徑，最後一定寫
            csv_wr.writerow([frame_id, visible,
            ...
            cv2.circle(img, (int(x_est), int(y_est)), 5, (0, 255, 0), -1)
            for i in range(1, len(traj)):
                cv2.line(img, traj[i-1], traj[i], (0, 255, 255), 2)
            writer.write(img)            
   

    cap.release()
    writer.release()
    print('[Info] 無論有沒有框到球，CSV 已輸出：', TXT_OUT)
